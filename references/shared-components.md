# Visual HTML — 共享组件语义契约与参考 DOM (Shared Components)

> **架构契约说明**
> 本文件定义 AI 解析长篇文本后可选用的语义组件、它们必须表达的内容关系，以及用于风格脚手架覆盖测试的参考 DOM。
> 组件的 5 阶段顺序和全量示例只约束创建或扩展风格时的脚手架；实际文章按原文结构和内容语义选用组件，不要求固定顺序或全量出现。
> 下方 HTML、类名与内部元素都是参考实现，不规定实际页面的视觉、布局或内部 DOM。目标风格决定组件的最终形态，同时必须保留对应组件的内容契约。

---

## 目录索引：风格脚手架的标准 5 阶段参考 DOM + 可选外挂组件
...
- **语义场景**：用于长页面的段落或章节开头。
- **内容契约**：保留章节标题及其原有层级；章节编号与装饰标记只在原文或目标风格需要时使用。
...
- **内容契约**：保留原文标题树与正文关系；导读只在原文已有或用户要求摘要时生成。
...
- **内容契约**：保留结论或提醒的正文、适用范围和依据；不得凭空提高警示等级。
...
- **内容契约**：完整保留段落、标题、列表、引用与内联语义，是无法映射到其他组件内容的保真容器。
...
- **内容契约**：保留完整代码或命令、语言/上下文（如原文可确认）和可复制的纯文本；复制交互与语法标记按目标风格需要提供。
- **参考 DOM**：上方的 `.code-block`、顶栏和 Token 类仅为脚手架示例，目标风格可采用其他内部结构。
- **降级契约**：无法使用专用代码组件时，保留原生 `<pre><code>` 的可读文本与横向滚动保护。
...
- **内容契约**：保留项目标题、说明和原有的并列或优先关系；不预设项目数量、编号、卡片或推荐状态的形态。
...
- **内容契约**：保留特性主张、说明和已有媒体上下文；没有媒体时不创建虚假占位内容。

---

## 10. Flowchart & Diagrams (流程图、系统架构与 Mermaid 图表引擎)

图解的内容契约是保留节点、关系、方向和标签。SVG、Mermaid 或其他表达方式由图的复杂度和目标风格决定；下方两种实现均为参考：
...
- **内容契约**：保留可比维度、各方数据、单位和证据；没有原始依据时不得标记推荐项。
...
- **内容契约**：保留轮次关系，并清晰区分提问、分析、建议、待查事实与用户决策。
- **呈现原则**：清晰区分提问、分析、推荐与用户决策，具体排版、图形和材质完全由目标风格决定。
