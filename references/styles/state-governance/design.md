...
国企政务严谨汇报风（State Governance Blue）专为**中国国企、央企、政务信息化规划、数字化转型方案、严肃白皮书研报与高端咨询演示**打造。它立足于中国现代政企商务汇报的核心审美范式：**权威稳重、逻辑严密、层级井然、图表化叙事与清爽高可读性**。

全站以纯净通透的冷白（`#FFFFFF`）配合微冷浅灰蓝（`#F4F7FB`）为画布底色，以**国企经典深海蓝（`#103A71`）与工信科技蓝（`#1A56DB`）**为主行动色，辅以冰霜浅蓝（`#F0F6FF`）卡片底衬与蓝色虚线焦点框，彻底摒弃花哨浮夸的弥散光晕、非规范杂色与霓虹噪点，将视觉焦点完全聚焦于**业务逻辑、指标关系与推演架构**。

### 6 大 Core Visual DNA

1. **权威稳重的深蓝多阶渐变色谱 (Multi-tier Corporate Blue System)**：
   - 核心深海蓝（`#103A71` / `#0D3A78`）、科技宝蓝（`#1A56DB` / `#2563EB`）、蔚蓝（`#3B82F6`）、浅天蓝（`#60A5FA` / `#93C5FD`）、浅冰蓝（`#EFF6FF` / `#F0F6FF`）与纯净冷白（`#FFFFFF`）构成严谨、富有秩序感的蓝白色彩韵律。
2. **贯穿性深蓝标题锚点（Anchor Headline & Primary Line）**：
   - 每一个章节或幻灯片标题下方，贯穿一条坚实洗练的 **1.5px~2px 权威深蓝基线**（`#103A71`），在全屏建立极强的第一视觉锚点；
   - 标题支持关键核心词汇（如“PPT”、“数字化”、“生命周期”）深蓝加粗强调，主副标题层级分明。
3. **双轨容器材质与蓝色虚线聚焦框（Dashed Focus Container）**：
   - 基础卡片采用 **冰霜浅蓝底衬（`#F0F6FF`）** 与纯白卡片（`#FFFFFF`）交替布局，搭配 6px~8px 严谨微圆角与极淡结构细线（`1px solid #BFDBFE`）；
   - 针对“工作开展详情”、“核心策略重点”与“关键诊断指标”，采用专属的 **蓝色虚线重点框（`1.5px dashed #2563EB`）**，形成清晰有力的视觉聚焦点。
4. **图表化叙事结构体系（Diagram-First Structured Storytelling）**：
   - 拒绝大段无条理的散装文本，大量使用**同心气泡数据聚类（Bubble Clustering）、生命周期环形切片（Lifecycle Ring Wheel）、三箭头推进器（`>>>` Chevron Stream）、多视角业务流转矩阵（Multi-Perspective Flow）与八维雷达策略对比图（8-Axis Radar）**。
5. **多视角胶囊徽章与分式算式卡（Perspective Badges & Formula Cards）**：
   - 支持多级纵向视角（用户视角、关键指标、商家视角）的深蓝/浅蓝胶囊标签；
   - 数字指标、公式换算（如 `转化率 = 成交人数 / 进房人数`）采用清晰的分数线卡片排版。
6. **参数等宽与高保真交付（Mono Precision & Strict Fidelity）**：
   - 百分比、金额、时间戳、考核指标统一使用 Mono 等宽排版；100% 忠实保留原文严谨论据。
...
| Background (Stage Canvas) | `rgb(226, 232, 240)` | `#E2E8F0` | `--bg-stage` | PPT 16:9 舞台外层沉浸灰底 |
...
:root {
  ...
  --shadow-slide: 0 12px 36px rgba(15, 23, 42, 0.12), 0 2px 8px rgba(15, 23, 42, 0.04);
...
## 3. Style-Owned Layout Contract (风格自有布局契约)

### Web Adaptation
- 采用 **直铺沉浸型与双轨交替底衬 (Direct Canvas & Dual Alternating Bands)**：
  - 页面主体由 `section` 纵向流式排布，奇数 `section` 使用纯白底色（`#FFFFFF`），偶数 `section` 使用微冷浅灰蓝底色（`#F4F7FB`）；
  - 每个 `section` 内部使用 `.wrap`（`max-width: 1140px; margin: 0 auto; padding: 0 24px;`）收敛居中；
  - 每一个 `section` 顶部必须包含带有贯通深蓝分割线（`1.5px solid var(--border-navy)`）的 `.section-head` 结构。

### PPT Adaptation
- 采用 **标准 16:9 固定舞台 (1280px × 720px Fixed Stage)**：
  - 背景为沉浸式灰底（`#E2E8F0`），每张幻灯片 `.slide` 为纯白卡片，带有立体投影（`var(--shadow-slide)`）；
  - 每张幻灯片固定包含三部分：
    1. 顶部贯穿线标题栏 `.slide-header`（带标题、英文小副标、类别徽章与深蓝贯穿线）；
    2. 幻灯片内容区域 `.slide-body`（占满剩余高度，内部使用网格或 Flex 分栏布局，严格不溢出）；
    3. 幻灯片页脚 `.slide-footer`（包含汇报主题名称、内部保密提醒与页码）。
  - 支持键盘快捷翻页（`ArrowLeft/Right` / `Space` / `PageUp/Down`）、全屏演示（`F` 键）以及 `@media print` 16:9 无边距打印导出。
...
4. **必须将大段文本图表化**：优先提炼为气泡图、环形生命周期、流程卡、药丸标签组、对比矩阵与公式框。
...
3. **严禁省略标题下方的深蓝分割线与 Eyebrow 标号**。
4. **严禁在正式汇报中使用不规范的网络表情符号 (Emoji)**，仅允许使用规范严谨的线性矢量图标或 SVG。
