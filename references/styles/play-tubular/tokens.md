...
13. **Flowchart (流程图与系统架构 - 轻量平衡卡片流)**:
    - 纯白大面板，4 个节点均采用统一规格的 **16px 大圆角轻量卡片**，内置精致的 Monospace 步骤药丸徽标（`01 / INPUT`, `02 / EXTRACT`），活跃聚焦节点（`03 / RENDER`）采用轻盈的微光浅蓝底色与 `1.5px` 电光蓝边框，去除笨重的发光与杂乱彩球，连接线采用轻快干练的细箭头管道，整体轻盈透亮、视觉平衡。
