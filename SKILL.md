...
| **`scene-distillation-zine`** | **手作刊物风** | 棉纸暖底 + 孔版钴青蓝 + 撕纸纤维毛边 + 半调网点 + 打字机题记与古法朱砂印 | 独立刊物、摄影与艺术评论、深度调研手札、文化随笔、诗意汇报与设计白皮书 |
...
2. **基于参考材料的单次生成**：默认 `Inspiration` 模式提炼可迁移规则（Web 基于 [`references/web/base-scaffold.html`](references/web/base-scaffold.html)，PPT 遵循 16:9 契约），不复制原文、Logo 与专有资产。
...
- Web 输出遵循全量保真；PPT 输出必须遵守演示媒介规律，先经过大纲提炼与用户确认（按时长规划页数），主屏聚焦单一论点与核心结论，论述细节转为演讲提词，密集参数与长代码放入附录页，不得无声明丢弃。
